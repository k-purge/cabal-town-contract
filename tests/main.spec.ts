...
import { hex as edgedWalletHex } from "../build/edged-case-jetton-wallet.compiled.json";
import { Blockchain } from "@ton-community/sandbox";
import { MainContract } from "../wrappers/MainContract";

describe("Test jetton minter and wallet", () => {
  ...
  it("test purchase token failed due to insufficient ton", async () => {
  ...
  it("test mint token success", async () => {
    ...
    const getData = await myContract.getData();
    expect(getData.supply).toEqual(10000000000000000n + 100000000000n);
  });
...
describe("Test edged cases on jetton minter and wallet", () => {
  const JETTON_WALLET_CODE = Cell.fromBoc(Buffer.from(edgedWalletHex, "hex"))[0];
  const adminAddress = Address.parse(
    "0QAOc7gW0Si6uEifcV-jLqR8dxFIHxiHtPxOkgTDXc9eR9n6"
  );
  ...
  it("test purchase token failed due to jetton wallet throw", async () => {
    const body = beginCell()
      .storeUint(31, 32) // op code 31 -> buy jetton
      .storeUint(1, 64)
      .storeCoins(new BN("1000000000"))
    ...
    latestSupply = getData.supply;
    jettonOwned = getData.supply;
  });

  it("test sell token failed due to jetton wallet throw", async () => {
    const body = beginCell()
      .storeUint(51, 32) // op code 51 -> sell jetton
      .storeUint(1, 64)
      .storeCoins(jettonOwned)
  ...
  it("test mint token to be failed", async () => {
    ...
    const getData = await myContract.getData();
    expect(getData.supply).toEqual(latestSupply);
    expect(getData.totalSupply).toEqual(100000000000000000000n);
  });
  ...
  it("test mint token failed due to jetton wallet throw", async () => {
    const body = beginCell()
      .storeUint(21, 32) // op code 21 -> mint jetton
      .storeUint(1, 64) // queryid
      .storeAddress(adminAddress)
